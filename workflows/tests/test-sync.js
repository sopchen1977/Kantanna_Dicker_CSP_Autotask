...
assert.strictEqual(nodes['Units Decision'][0].json.delta, 51);
assert.strictEqual(nodes['Units Decision'][0].json.cs_id, 8001);

nodes['Adjust Result'] = runNode('adjust-result.js', [{ json: { itemId: 6001 } }], nodes);
assert.strictEqual(nodes['Adjust Result'][0].json.adjust_ok, true);
...
assert.strictEqual(n2['Units Decision'][0].json.delta, 0);
