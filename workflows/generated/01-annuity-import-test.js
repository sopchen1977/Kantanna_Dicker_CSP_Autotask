...
const uploadForm = trigger({
  ...
  config: {
    ...
    position: [-380, 0],
    parameters: {
      formTitle: 'Dicker Data CSP Monthly Import · TEST',
...
const noteImport = sticky(
  '## 01 · Annuity Import\nUpload both monthly Dicker Data files. Lines are matched on Subscription ID + Stock Code and upserted, so custom sell prices and include/exclude choices saved in the portal survive re-imports.\n\n**Pilot filter:** edit PILOT_CUSTOMERS at the top of the "Parse Subscription Lines" node (this stack: ['Galilee']; matched as a substring, empty list = all customers).',
  [uploadForm, normalizeUploads],
  { color: 4 }
);

export default workflow('kantanna-csp-01-import-test', '01 · Annuity Import (Upload) · TEST')
  .add(uploadForm)
  .to(normalizeUploads)
